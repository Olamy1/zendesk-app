...
#   • Automatically BYPASSES in safe environments (local, test, unit, integration, e2e)
...
SAFE_ENVS = {"local", "test", "unit", "integration", "e2e"}
...
class TokenAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # 🚨 PATCH: Explicitly handle missing Authorization header
        if not auth_header:
        ...
        # 🚨 PATCH: Explicitly handle invalid or mismatched token
